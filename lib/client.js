	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"delete.session": "删除对话",
			"delete.session.desc": "将永久删除“{name}”的聊天记录与本地日志，删除后无法恢复。",
		...
		const en = {
			...
			"delete.session.desc": "This permanently deletes “{name}” and its local chat log. This cannot be undone.",
		...
		function apply(ctx) {
			...
			ctx.slots.inject("sidebar.workspaces", () => {
				try {
					...
					console.warn("[dsh-session-delete] sidebar.workspaces registration failed:", error);
					return () => {};
				}
			});
